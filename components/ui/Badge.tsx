...
export function Badge({ children, variant = 'teal', size = 'md', className = '' }: BadgeProps) {
  const variantStyles: Record<string, string> = {
    ...
    secondary: 'bg-neutral-border text-neutral-mid',
    ...
    error:     'bg-red-100 text-error dark:bg-red-900/30 dark:text-red-400',
  ...
  return (
    <span
      className={`inline-flex items-center font-semibold rounded-full ${variantStyles[variant] ?? variantStyles.teal} ${sizeStyles[size]} ${className}`}
    >
      {children}
    </span>
  );
