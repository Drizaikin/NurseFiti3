-- =============================================================================
-- Migration: Tutor-controlled price negotiation settings
-- =============================================================================
-- Each tutor individually decides whether students can propose a lower rate.
-- The tutor also sets their own floor (minimum accepted amount), which must
-- be at least KSh 1,000. Defaults to off / 1000 for all existing tutors.
-- =============================================================================

ALTER TABLE tutor_profiles
  ADD COLUMN IF NOT EXISTS allow_price_negotiation BOOLEAN  NOT NULL DEFAULT FALSE,
  ADD COLUMN IF NOT EXISTS min_negotiated_rate     INTEGER  NOT NULL DEFAULT 1000;

-- Enforce the platform-wide floor: min_negotiated_rate must be >= 1000
ALTER TABLE tutor_profiles
  DROP CONSTRAINT IF EXISTS chk_min_negotiated_rate;

ALTER TABLE tutor_profiles
  ADD CONSTRAINT chk_min_negotiated_rate
    CHECK (min_negotiated_rate >= 1000);

COMMENT ON COLUMN tutor_profiles.allow_price_negotiation IS
  'When TRUE, students may propose a lower rate when booking this tutor.';

COMMENT ON COLUMN tutor_profiles.min_negotiated_rate IS
  'The minimum amount (KSh) this tutor will accept from a student proposal. Must be >= 1000.';

-- Update create_booking_atomic to enforce the TUTOR''s negotiation settings.
-- The function now:
--   1. Fetches the tutor's allow_price_negotiation flag and min_negotiated_rate.
--   2. Raises negotiation_not_allowed if student proposes an amount but the tutor has disabled it.
--   3. Raises proposed_amount_too_low if the proposed amount is below the tutor's floor.
